...
# 脚本位于 ComfyUI/script，默认工作目录使用其上级 ComfyUI 根目录
COMFY_DIR="${COMFY_DIR:-$(cd "$SCRIPT_DIR/.." && pwd)}"
COMFY_MAIN="main.py"
LISTEN_ADDR="0.0.0.0"
BASE_PORT="8888"
USE_CONDA_RUN=0
...
# 非交互模式端口占用处理策略（适合 systemd）
# 1 = 尝试自动终止“看起来像 ComfyUI 的进程”来释放端口（更强势）
# 0 = 不终止，自动换端口
AUTO_KILL_PORT_OCCUPIERS="${AUTO_KILL_PORT_OCCUPIERS:-1}"
...
kill_comfy_pids_on_port() {
    local port="$1"
    local pids
    pids="$(list_port_pids "$port" | xargs 2>/dev/null)"
    [ -z "$pids" ] && return 1
    ...
    for pid in $pids; do
        ...
        # 只杀命令行中包含 comfyui-<port> 的进程，避免误伤其他服务
        if echo "$cmd" | grep -qE '(^|[[:space:]])comfyui-[0-9]+([[:space:]]|$)'; then
            echo "WARN: 自动终止占用端口 ${port} 的 ComfyUI 进程: PID=${pid} CMD=${cmd}" >&2
            kill "$pid" 2>/dev/null || true
            killed_any=1
        else
            echo "WARN: 端口 ${port} 被占用，但进程不匹配 ComfyUI，跳过终止: PID=${pid} CMD=${cmd}" >&2
        fi
    done
...
resolve_single_port() {
    ...
    while port_in_use "$target_port"; do
        local killed=0
        if [ -n "$label" ]; then
            echo "WARN: [$label] 端口占用: $target_port" >&2
        else
        ...
        if [ -n "$pids" ]; then
            ...
            if [ -t 0 ]; then
                ...
                if [ "$AUTO_KILL_PORT_OCCUPIERS" = "1" ]; then
                    if kill_comfy_pids_on_port "$target_port"; then
                        killed=1
                    fi
                fi
            fi
        ...
        if [ "$killed" = "0" ]; then
            ...
            if [ -n "$label" ]; then
                echo "WARN: [$label] 自动尝试端口: $target_port" >&2
            else
...
run_comfy_named() {
    ...
    # ss -ltnp 常见展示的是 comm(PR_SET_NAME)，仅改 argv0 不一定稳定可见。
    # 这里用 Python 包装：在同一进程内设置 comm，然后 runpy 执行 main.py。
    # comm 最长 15 字节，因此将名字压缩为 comfyui-<port>。
    local short_name
    short_name="$(echo "$proc_title" | sed -E 's/^comfyui:([0-9]+).*/comfyui-\1/; s/[^a-zA-Z0-9._-]/_/g')"
    short_name="${short_name:0:15}"

    local py_wrapper
    # 注意：
    # - 不要用 exec -a 改 argv0（会导致某些 conda/python 组合 sys.executable 解析失败，进而报 encodings 缺失）
    # - 作为脚本运行时 sys.path[0] 会是当前脚本目录，因此这里显式把 ComfyUI 根目录插入 sys.path，避免找不到 comfy
    # 把 Python 代码通过 stdin 传给 `python -`，避免 `ps`/后台把整段 -c 代码展示出来（命令行会短很多）
    py_wrapper='import ctypes,os,runpy,sys
...
runpy.run_path(os.path.join(comfy_dir,"main.py"), run_name="__main__")
'

    if [ "$USE_CONDA_RUN" = "1" ]; then
        # 只设置 comm(PR_SET_NAME) 便于排查端口占用。
        CONDA_NO_PLUGINS=true conda run -n GAOSHIQING python - "$short_name" "${args[@]}" <<<"$py_wrapper"
    else
        python - "$short_name" "${args[@]}" <<<"$py_wrapper"
    fi
}
...
start_instance_bg() {
    ...
    if [ "$gpu" = "3" ]; then
        ...
        run_comfy_named "comfyui:${port}:cpu" --cpu --listen "$LISTEN_ADDR" --port "$port" > "$log_file" 2>&1 &
    elif [ "$gpu" = "2" ]; then
        echo "启动实例 ${index_label}: 端口 $port, 所有 GPU"
        unset CUDA_VISIBLE_DEVICES
        run_comfy_named "comfyui:${port}:all" --listen "$LISTEN_ADDR" --port "$port" > "$log_file" 2>&1 &
    else
        echo "启动实例 ${index_label}: 端口 $port, GPU $gpu"
        CUDA_VISIBLE_DEVICES="$gpu" run_comfy_named "comfyui:${port}:gpu${gpu}" --listen "$LISTEN_ADDR" --port "$port" > "$log_file" 2>&1 &
    fi
    echo "日志: $log_file"
}
...
start_two_instance_mode() {
    ...
    CUDA_VISIBLE_DEVICES="$gpu_a" run_comfy_named "comfyui:${port1}:gpu${gpu_a}" --listen "$LISTEN_ADDR" --port "$port1" > "$log1" 2>&1 &
    echo "启动实例 B: 端口 $port2, GPU $gpu_b"
    CUDA_VISIBLE_DEVICES="$gpu_b" run_comfy_named "comfyui:${port2}:gpu${gpu_b}" --listen "$LISTEN_ADDR" --port "$port2" > "$log2" 2>&1 &
    echo ""
...
if [ "$RUN_MODE" = "custom" ]; then
    ...
    for idx in "${!INSTANCE_PORTS[@]}"; do
        ...
        local_gpu="${INSTANCE_GPUS[$idx]}"
        resolved_port="$(resolve_single_port "$local_port" "实例$((idx+1)) 原端口${local_port}")"
        while [[ " ${RESOLVED_INSTANCE_PORTS[*]} " == *" ${resolved_port} "* ]]; do
            resolved_port=$((resolved_port+1))
            resolved_port="$(resolve_single_port "$resolved_port" "实例$((idx+1)) 原端口${local_port}")"
        done
...
if [ "$RUN_MODE" == "custom" ]; then
    ...
    if [ "$USE_CPU" = "1" ]; then
        ...
        write_run_summary_file
        unset CUDA_VISIBLE_DEVICES
        run_comfy_named "comfyui:${BASE_PORT}:cpu" --cpu --listen "$LISTEN_ADDR" --port "$BASE_PORT"
    elif [ "$GPU_CHOICE" = "2" ]; then
        echo "使用所有GPU设备"
        RUN_SUMMARY_LINES=("端口 ${BASE_PORT} -> ALL_GPU | 日志: 当前终端标准输出")
        write_run_summary_file
        run_comfy_named "comfyui:${BASE_PORT}:all" --listen "$LISTEN_ADDR" --port "$BASE_PORT"
    else
        echo "使用单卡: $GPU_CHOICE"
        RUN_SUMMARY_LINES=("端口 ${BASE_PORT} -> GPU ${GPU_CHOICE} | 日志: 当前终端标准输出")
        write_run_summary_file
        export CUDA_VISIBLE_DEVICES="$GPU_CHOICE"
        run_comfy_named "comfyui:${BASE_PORT}:gpu${GPU_CHOICE}" --listen "$LISTEN_ADDR" --port "$BASE_PORT"
    fi
fi
